...
import { renderAnswerKeyPdf } from "@/lib/export/pdf";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const buffer = await renderAnswerKeyPdf(model);

    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${slug(paper.title)}-answer-key.pdf"`,
      },
    });
  } catch (error) {
